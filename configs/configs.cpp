...
void C_ConfigManager::setup()
{
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].adaptive_ps, false, std::to_string(i) + crypt_str("Ragebot.adaptive_ps"));
		setup_item(&g_cfg.ragebot.weapon[i].ignore_limbs, false, std::to_string(i) + crypt_str("Ragebot_ignore_limbs"));
	}
	...
	setup_item(&g_cfg.antiaim.while_shot, false, crypt_str("Antiaim.while_shot"));
	...
	setup_item(&g_cfg.esp.scope_type, 1, crypt_str("Misc.scope_type"));
	setup_item(&g_cfg.esp.new_scope_color, Color(255, 255, 255), crypt_str("Esp.new_scope_color"));
	...
	setup_item(&g_cfg.esp.thunder, false, crypt_str("Esp.thunder"));
	...
	setup_item(&g_cfg.esp.penetration_reticle2, false, crypt_str("Esp.penetration_reticle2"));
	...
	setup_item(&g_cfg.esp.keybind_list, false, crypt_str("Esp.keybind_list"));
	...
	setup_item(&g_cfg.misc.autoaccept, false, crypt_str("Misc.autoaccept"));
	//setup_item(&g_cfg.misc.chams_on_shot_enable, false, crypt_str("Misc.chams_on_shot_enable"));
	setup_item(&g_cfg.misc.chams_on_shot_type, 0, crypt_str("Misc.chams_on_shot_type"));
	setup_item(&g_cfg.misc.chams_on_shot_color, Color(255, 255, 255), crypt_str("Misc.chams_on_shot_color"));
	setup_item(&g_cfg.misc.ragdol_force, 0, crypt_str("Misc.ragdol_force"));
	setup_item(&g_cfg.misc.ragdol_force_x, 0, crypt_str("Misc.ragdol_force_x"));
	setup_item(&g_cfg.misc.ragdol_force_y, 0, crypt_str("Misc.ragdol_force_y"));
	...
	setup_item(&g_cfg.misc.check_for_null_strafes, false, crypt_str("Misc.null_strafes"));
	...
	setup_item(&g_cfg.misc.fake_prime, false, crypt_str("Misc.fake_prime"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\Lolihook\\Configs\\") + config;
		}
	...
	zfolder = std::string(path) + ("\\Lolihook\\");
	zfolder += ("\\Inventory\\");
...
void C_ConfigManager::pre_safe()
{
	...
	std::string folder;
	SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path);
	folder = std::string(path) + ("\\Lolihook\\");
	folder += ("\\Inventory\\");
