		...
		if (current_shot != g_ctx.shots.end())
		{
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					std::string reason = crypt_str("spread");
					...
					if (current_shot->impact_hit_player)
					{
						...
						if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
							reason = crypt_str("resolver [resolver: "); //+ std::to_string(aim::get().last_target[current_shot->last_target].record.resolveValue) + crypt_str("]");
					}
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						if (current_shot->occlusion)
							reason = crypt_str("occlusion");
						else if (current_shot->shot_info.hitchance == 100)
							reason = crypt_str("prediction error");
						else
							reason = crypt_str("spread [Hitchance: ") + std::to_string(current_shot->shot_info.hitchance) + crypt_str("]");
					}
					eventlogs::get().add(crypt_str("missed shot to: ") + (std::string)current_shot->shot_info.target_name + crypt_str(" in: ") + (std::string)current_shot->shot_info.client_hitbox + crypt_str(" reason: ") + reason);
				}
			}
