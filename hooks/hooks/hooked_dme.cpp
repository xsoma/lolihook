...
void __stdcall hooks::hooked_dme(IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* bone_to_world)
{
	...
	auto is_weapon = strstr(name, "weapons/v_") && !strstr(name, "arms") && (g_cfg.esp.weapon_chams || g_cfg.misc.customchams);
	...
	if (m_engine()->IsConnected() && m_engine()->IsInGame() && g_cfg.misc.customchams)
	{
	...
	CreateMaterial(true, crypt_str(R"#("VertexLitGeneric" 
			...
			auto material = materials_knife_custom[g_cfg.misc.customknife];
			auto double_material = materials[6];
			...
			if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial() && knowledge.is_knife)
			{
				...
				{
					g_cfg.misc.knife_color_custom[0] / 255.0f,
					g_cfg.misc.knife_color_custom[1] / 255.0f,
					g_cfg.misc.knife_color_custom[2] / 255.0f
				};
