...
#include "../../cheats/misc/eventlog.h"
...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (reload_fonts())
	{
		...
		fonts[SUBTABWEAPONS] = create_font(crypt_str("astriumwep"), 13, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[KNIFES] = create_font(crypt_str("astriumwep"), 13, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[GRENADES] = create_font(crypt_str("astriumwep"), 20, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[INDICATORFONT] = create_font(crypt_str("Calibri"), 26, 100, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		...
		fonts[FONT_CRIUS] = create_font(crypt_str("Verdana"), 13, FW_MEDIUM, FONTFLAG_DROPSHADOW);
		fonts[RADIX_FONT] = create_font(crypt_str("Verdana"), 12, FW_MEDIUM, FONTFLAG_ANTIALIAS);
		...
		if (g_ctx.available())
		{
			...
			if (g_cfg.player.enable && g_cfg.esp.removals[REMOVALS_SCOPE] && g_ctx.globals.scoped && weapon->is_sniper())
			{
				...
				if (g_cfg.esp.scope_type == 0)
				{
					...
					render::get().line(w / 2, 0, w / 2, h, Color(r, g, b, a));
					render::get().line(0, h / 2, w, h / 2, Color(r, g, b, a));
				}
				if (g_cfg.esp.scope_type == 1)
				{
					static int w, h;
					m_engine()->GetScreenSize(w, h);
			...
			otheresp::get().penetration_reticle2();
