...
void lagcompensation::update_player_animations(player_t* e)
{
...
#endif
	{
		...
		animstate->m_flGoalFeetYaw = math::normalize_yaw(animstate->m_flEyeYaw + e->get_max_desync_delta());
		...
		/*if (e->m_vecVelocity().Length2D() > 35.f && e->m_vecVelocity().Length2D() < 210.f)
		{
			switch (g_ctx.globals.missed_shots[e->EntIndex()] % 4)
			{
			case 0:
