...
void fakelag::Fakelag(CUserCmd* m_pcmd)
{
	...
	static auto random_factor = min(rand() % 15 + 1, g_cfg.antiaim.triggers_fakelag_amount);
	...
	else
	{
		if (g_cfg.ragebot.enable && g_ctx.globals.current_weapon != -1 && !g_ctx.globals.exploits && g_cfg.antiaim.fakelag && g_cfg.antiaim.fakelag_enablers[FAKELAG_PEEK] && g_cfg.antiaim.triggers_fakelag_amount > 6 && !started_peeking && velocity >= 5.0f && !on_land)
		{
			...
			for (auto i = 1; i < m_globals()->m_maxclients; i++)
			{
				...
				if (predicted_data.valid())
				{
					...
					if (!data.valid())
					{
						random_factor = min(rand() % 15 + 1, g_cfg.antiaim.triggers_fakelag_amount);
		...
		else if (!g_ctx.globals.exploits && g_cfg.antiaim.fakelag && !on_land)
		{
			...
			else
			{
				...
				random_factor = min(rand() % 15 + 1, g_cfg.antiaim.fakelag_amount);
				switch_ticks = !switch_ticks;
		...
		else if (!g_ctx.globals.exploits && g_cfg.antiaim.fakelag && !g_cfg.antiaim.while_shot && g_ctx.globals.aimbot_working) {
			if (choked < max_choke)
				g_ctx.send_packet = false;
			else
			{
...
void fakelag::Createmove()
{
	...
	if (!m_gamerules()->m_bIsValveDS() && m_clientstate()->iChokedCommands <= 15)
	{
