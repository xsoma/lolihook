...
void eventlogs::events(IGameEvent* event) 
{
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_HIT] && !strcmp(event->GetName(), crypt_str("player_hurt")))
	{
		...
		{
			ss << crypt_str("Hurt ") << userid_info.szName << crypt_str(" in ") << get_hitgroup_name(event->GetInt(crypt_str("hitgroup"))) << crypt_str(" for ") << event->GetInt(crypt_str("dmg_health"));

			add(ss.str());
		}
		else if (userid_id == m_engine()->GetLocalPlayer() && attacker_id != m_engine()->GetLocalPlayer())
		{
			ss << crypt_str("Player ") << attacker_info.szName << crypt_str(" hurt you for ") << event->GetInt(crypt_str("dmg_health")) << crypt_str(" in ") << get_hitgroup_name(event->GetInt(crypt_str("hitgroup")));
...
#else
		auto log = crypt_str("[ \x0C^LOLIHOOK \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
#else
